...
def login():
    ...
    if request.method=="POST":
        ...
        if User_name==user_name and Password==Pass:
...
@ma.route("/student",methods=["GET","POST"])
def user():
    if request.method=="POST":
        ...
        return render_template("student.html",data=student_info,student=student_list)
    return render_template("student.html",data=student_info,student=student_list)
@ma.route("/add_student",methods=["GET","POST"])
def add_student():
    if request.method=="POST":
        Name=request.form.get("Name")
        Age=request.form.get("Age")
        ...
        # Marks_list.append(mark1)
        # Marks_list.append(mark2)
        # Marks_list.append(mark3)
        # Marks_list.append(mark4)
        # Marks_list.append(mark5)
        students_dict={}
        students_dict.update({"Name":Name})
        students_dict.update({"Age":Age})
        students_dict.update({"RollNo":RollNo})
        students_dict.update({"Marks":Mark_list})
        students_new.append(students_dict)
        return redirect(url_for("user"))
    return render_template("add_student.html",students=students_new,student=student_list)
